...
namespace CreatureModule
{
	...
	class Utils
	{
		public static Dictionary<string, object> LoadCreatureJSONData(string filename_in)
		{
			...
			ret_dict = MiniJSON.Json.Deserialize(text) as Dictionary<string, object>;
		...
		public static Dictionary<string, object> LoadCreatureJSONDataFromString(string text_in)
		{
			...
			Dictionary<string, object> ret_dict = null;
			ret_dict = MiniJSON.Json.Deserialize(text_in) as Dictionary<string, object>;
			//ret_dict = (Dictionary<string, object>)raw_obj;
		...
		{
			meta_data.clear();
			var json_dict = MiniJSON.Json.Deserialize(json_text_in) as Dictionary<string, object>;
		...
		public static float[] getFloatArray(System.Object raw_data)
		{
			...
			System.Object[] cur_obj = (object[])raw_data;
		                                               ...
		                                               string key)
		{
			if(data[key].GetType() == typeof(double[]))
			{
				...
				for (int i = 0; i < num_dbl_points; i++) 
				{
					int cur_index = i * 2;
					ret_double_list.Add(
								new XnaGeometry.Vector2((float)double_obj[0 + cur_index],
                                                        (float)(double_obj[1 + cur_index])) );
			...
			System.Object[] cur_obj = (object[])data[key];
			...
			for (int i = 0; i < num_points; i++) 
			{
				int cur_index = i * 2;
				ret_list.Add(
					new XnaGeometry.Vector2((float)((double)cur_obj[0 + cur_index]),
                                        (float)((double)cur_obj[1 + cur_index])) );
		                                           	   ...
		                                           	   string key)
		{
			...
			Tuple<int, int> ret_times = new Tuple<int,int>(start_time,end_time);
			return ret_times;
		}
